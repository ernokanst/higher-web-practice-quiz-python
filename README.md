...
API построен на основе Django REST Framework. Эндпоинты реализованы через сервис-ориентированные `APIView` и явные маршруты в `quiz/urls.py`. Валидация выполняется в сериализаторах и валидаторах.
...
│   ├── constants.py               # Константы тестов проекта
│   ├── test_services.py
│   └── test_views.py
...
#### Вопросы
- Отображение ID, превью текста, квиза, категории и сложности
